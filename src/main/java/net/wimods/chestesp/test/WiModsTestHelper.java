...
public enum WiModsTestHelper
{
	...
	private static boolean clickButtonInWidget(AbstractWidget widget, int x,
		int y)
	{
		if(widget instanceof Button button && button.getX() == x
			&& button.getY() == y)
		{
			button.onPress();
			return true;
	...
	public static void assertOneItemInSlot(int slot, Item item)
	{
		submitAndWait(mc -> {
			...
			if(!stack.is(item) || stack.getCount() != 1)
				throw new RuntimeException(
					"Expected 1 " + item.getDescriptionId() + " at slot " + slot
						+ ", found " + stack.getCount() + " "
						+ stack.getItem().getDescriptionId() + " instead");
		});
	}
